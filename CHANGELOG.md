...
- 本次为一版“搜索与空间加载稳定性修复 + 设置搜索升级 + 动效与导航收口 + 番剧播放链路补强”的维护更新。
...
- 详情页顶部返回动作的离场状态管理进一步收口，减少切视频后旧页面状态残留对新页面点击的影响。
...
- 本次为“外观与导航体验收口 + 搜索与播放器稳定性修复 + 离线播放细节补强”的维护更新。

### 外观、主题与导航体验收口
- 主题系统继续向统一视觉策略收口：Miuix 动态色会更准确跟随系统 / 浅色 / 深色模式，静态 MD3 方案也会基于主色派生更明确的 secondary、tertiary 与表面层级，不再长期停留在过于固定的默认面板色。
...
### 播放器、竖屏详情与弹幕设置升级
- 竖屏详情页继续向更稳定的共享播放器接管方案收口：官方风格的 inline 竖屏详情、竖屏全屏切换和退出动画会更顺，减少播放器在不同宿主之间切换时的闪烁感。
...
- 平板视频详情、影院布局与视频内容区继续做了间距、排布和行为收口，大屏下的播放器与信息区衔接更稳定。
...
- 本次为“普通视频起播/回退链继续向 PiliPlus 对齐 + 播放器交互修复 + 玻璃/弹幕可读性修复 + 构建链稳定性优化”的维护更新。

### 普通视频起播与播放地址获取稳定性
- 普通视频首个 `WBI` 请求现在固定以更稳定的 `1080P (qn=80)` 作为起播入口，再交给播放层按实际 `DASH` 轨道选画质，更接近 PiliPlus 的起播策略。
...
- 本次为“Android Native / Miuix 视觉收口 + 直播/后台播放稳定性修复 + 弹幕能力增强”的维护更新。

### 首页底栏、设置页与液态玻璃收口
- 首页底栏新增更完整的 Android Native / Miuix 壳层方案，底栏容器、选中指示器和分隔策略统一收口到共享材质配方，整体观感更贴近原生悬浮底栏。
...
### 弹幕设置、过滤与同步增强
- 弹幕配置新增字重、行高、滚动时长、固定速度、静态停留时长、海量模式等更细控制项，滚动层和顶部/底部弹幕的排布会随视口尺寸与显示区域更合理地收口。
...
- 本次为“发布来源校验强化 + 播放链路向 PiliPlus 进一步对齐 + 播放恢复与无声问题修复”的功能更新版本。
...
### 普通视频与番剧播放链路继续向 PiliPlus 对齐
- 普通视频 `playurl` 主路径继续收口到 Web/WBI，不再优先走 APP `access_token` 接口，减少高画质接口风控和空 payload 带来的不稳定切换。
- 普通视频 fallback 继续精简到单条 Web/WBI 主路径，更接近 PiliPlus 的实际取流策略，不再在主链路里额外回退到 legacy / guest playurl。
- 番剧 `playurl` 已切到更接近 PiliPlus 的 `web/v2` 路径，并补齐 `cid / bvid / season_id / try_look / voice_balance / gaia_source / isGaiaAvoided / web_location` 等上下文参数。

### 普通视频画质选择完全按 PiliPlus 思路收口
...
- 本次为“隐私保护强化 + 播放器稳态收口 + 交互与下载细节修复”的维护更新。
...
- 播放心跳现在会上报更准确的会话开始时间、实际观看时长与当前播放进度，并在暂停、离开页面、切换视频或切换分P时主动补发收尾心跳，历史记录和观看进度统计更完整。
- 音频模式页只会在前一个页面仍然是有效视频页时才复用播放器 `ViewModel`，减少返回栈已销毁后误共享旧播放状态的问题。

### 评论区、卡片与交互细节
- 评论区等级徽章切换为更接近 PiliPlus 风格的像素资源图，并补上 `6级高能会员` 的专属徽章显示。
...
- 本次为“设置页动效收口 + 自适应 DASH/AV1 回退增强 + 播放器揭帧更顺滑”的维护更新。
...
- 本次为“液态玻璃连续调节 + 播放器横竖屏与 PiP 收口 + 投屏/历史稳定性修复”的维护更新。
...
- 前后台切换时的暂停、继续播放、恢复音量和后台音频标记逻辑进一步收口，进入小窗、画中画和真正离开应用时的行为边界更清晰，减少误暂停、误恢复和回前台无声的情况。
...
- 本次为“播放器前后台恢复与进度同步增强 + 播放取流稳定性修复 + 主题/插件细节收口”的维护更新。
...
- 竖屏分页播放器在页面切换、长按倍速、进度提交和本地显示同步上的行为继续收口，跨页切换时的倍速残留和 seek 位置抖动问题进一步减少。
...
- 弹幕管理器的内部协程作用域创建与切换逻辑进一步收口，减少作用域复用不当带来的观察链路问题。
...
- `后台播放模式`、画中画相关提示与开关文案继续收口；当关闭后台播放或启用“离开播放页后停止”时，相关模式会明确显示为已覆盖，减少设置之间互相打架的困惑。
- 视频详情页现在优先沿用入口卡片封面，减少从首页、动态、空间等入口进入时封面突变的问题；未主动播放前会保留封面并显示更明确的播放按钮，手动起播体验更接近 PiliPlus。
...
- `MD3` 首页顶部搜索栏的回弹显隐节奏继续收口；轻微反向滚动时搜索栏会更早开始恢复显示，顶部折叠反馈更跟手。
...
- 评论主列表、楼中楼详情、回复入口、图片预览和时间戳跳转现在共用同一条展示路径，后续行为收口更容易保持一致。
...
### 首页顶部、搜索折叠与触感反馈
- 首页顶部搜索栏折叠距离、显隐节奏与状态栏融合策略继续收敛，顶部区域在回到列表顶端前不再过早回弹，折叠后的整体性更稳定。
...
- 手机端自动旋转进入全屏的方向策略继续收敛：默认保持竖屏，只有传感器达到更明确的横屏姿态时才进入横屏；已经处于横屏时也会更稳地保持横屏，减少抖动。
...
### 测试与回归
- 新增或补强首页顶部交互、顶部几何、动态布局策略、竖屏全屏手势和手机端方向策略等单元测试，覆盖本次交互收口涉及的关键规则。

## v7.0.0 (2026-03-20)

### 版本信息
- 版本号从 `7.0.0 RC2` 升级到 `7.0.0`，`versionCode` 升级到 `120`。
- 本次为 `7.0.0` 正式版，合并了 `Beta1` 至 `Beta5` 与 `RC / RC2` 的关键更新，重点收口界面预设、导航链路、播放后台与整体稳定性。
...
- 听视频模式、离线播放、后台播放、通知栏/系统媒体中心控制与播放队列管理继续收口，前后切歌、后台控制与锁屏交互更稳定。
...
### 稳定性、工程化与测试
- 新增 `assembleFast / installFast`、更接近正式版的 `Dev` 变体，以及更收敛的 `Debug` 运行态配置，方便开发与回归验证。
...
- 本次为“首页液态玻璃与全局视觉收口 + 搜索深链与导航补齐 + 播放器后台降载优化”的发布候选更新。
...
- 本次为“入口链接统一导航 + 显示密度与字号调节 + 首页/动态/直播/投屏体验收口”的发布候选版本。

### 入口链接、WebView 与路由收口
- MainActivity 现在统一解析 `b23.tv / bilibili.com / bilibili://` 等入口链接，可直接识别视频、动态、UP 空间、直播间、番剧和音乐页，并在导航控制器就绪后再执行跳转，减少冷启动时的漏跳与错跳。
- 新增 `BilibiliNavigationTargetParser`，把分享文本、短链展开、站内 URL 和 Deep Link 的目标解析收敛到同一套逻辑。
...
- 设置页若干分组分隔线与图标着色继续按当前主题和预设收口，减少 iOS/MD3 预设切换时的视觉断层。
...
### 动态、UP 空间与站内内容页
- 动态富文本现在支持直接识别并点击链接，站内链接优先应用内打开，外链走系统浏览器；`@`、话题、表情与纯文本的渲染逻辑也统一收敛。
- 动态页针对用户筛选、风控/限流错误提示、用户动态加载时机和远端请求参数做了收口，降低切换用户时的空白、误重试和错误提示不准的问题。
...
- 播放相关细节继续收口，包括按小窗模式与退出策略决定唤醒锁模式，减少后台播放与功耗策略不一致的问题。

### 测试与稳定性
- 为入口链接解析、显示策略、动态富文本、列表外观、直播 tab 配色、投屏设备解析、播放功耗策略等新增或补强定向单测。
- 继续把首页、动态、空间、设置和播放器里的视觉/交互规则拆成可测试的 policy/helper，降低 RC 阶段继续收尾时的回归风险。
