...
- 本次为一版“评论区 PiliPlus 样式与抓取升级 + 评论富文本和操作完善 + 听视频封面视觉升级 + 播放中评论徽章稳定显示”的功能更新。
...
### 评论区展示
- 评论区等级标识统一改为 PiliPlus 风格像素等级徽章，并复用到个人页、空间页和侧边栏等用户信息位置。
- UP 主标识迁移为本地 PiliPlus 小徽章样式，评论、视频信息和播放器相关入口保持一致。
...
- 评论详情在竖屏弹层中撑满可用高度，相关回复与对话详情标题、排序入口和列表布局更接近 PiliPlus。
...
- 本次为“视频画质链路统一 + 动态/UP 空间对齐 PiliPlus + Material You 视觉校正”的综合更新版本。
