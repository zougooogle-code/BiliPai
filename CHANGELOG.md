...
- 优化首页与底栏的点击刷新逻辑 (下拉刷新动画优化)

## v4.2.3 (2026-01-25)

### 🐛 画中画修复

- 修复首页下滑返回桌面时错误进入系统 PiP 的问题
- 修复退出视频后仍有声音残留的问题
- 优化媒体通知清理逻辑
...
- 官方启动图 (Splash Wallpaper) 支持 (可从个人中心/设置配置)
- 访客模式优化 (引导登录)
...
- 新增 `BackgroundManager` 应用级后台状态管理
- 后台音频模式自动禁用视频轨道，CPU 降低 50-70%
- 前台恢复时自动恢复视频播放
- 添加 `lifecycle-process` 依赖支持 `ProcessLifecycleOwner`

### 🐛 Bug 修复

- 修复新手引导页图标显示为空白的问题（替换为标准 emoji）
